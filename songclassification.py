...
def calculate_score():
	with open('genres_v2.csv', 'r', encoding="utf8") as csv_file:
		...
		for row in reader:
			...
			score /= 5
			names[row[19]] = Song(row[19], row[18], float(row[0]),
								  float(row[1]), float(row[8]), float(row[10]))
			if score not in song_scores:
				song_scores[score] = [
					Song(row[19], row[18], float(row[0]), float(row[1]),
						 float(row[8]), float(row[10]))]
			else:
				song_scores[score].append(
					Song(row[19], row[18], float(row[0]), float(row[1]),
						 float(row[8]), float(row[10])))
...
def generate_playlist(genre_selections: list):
	...
	for key, value in song_scores.items():
		song_scores[key] = [elem for elem in value if
							elem.genre in genre_selections]
	...
	for key, value in song_scores.items():
		for song in value:
			...
			for saved in good_songs:
				similarity += (
					abs(song.danceability - names[saved[1]].danceability))
				similarity += (abs(song.energy - names[saved[1]].energy))
				similarity += (
					abs(song.liveliness - names[saved[1]].liveliness))
				similarity += (
					abs(song.tempo / 100 - names[saved[1]].tempo / 100))
			song.similarity = similarity
			if similarity < min_sim and (
					audience_score - 1 < key < audience_score + 1):
				min_sim = similarity
				min_song = song
	if min_song is None:
